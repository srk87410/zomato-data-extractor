...
const PRODUCT_ID = "62a7255919aab584ba2fb2e4";
var DEVICE_ID = "";
var DATA = {};
...
function downloadContent(url) {
  return new Promise((resolve, reject) => {
    fetch(
      ...
      .catch((err) => {
        console.log("fetchScript Error:", err);
        resolve(null);
      });
  });
}

function saveContentJS(url) {
  downloadContent(url).then((res) => {
    CONTENT_JS = res;
    chrome.storage.local.set({ script: CONTENT_JS });
    console.log("saveContentJS:", CONTENT_JS);
  });
}

function saveContentCSS(url) {
  downloadContent(url).then((res) => {
    CONTENT_CSS = res;
    chrome.storage.local.set({ style: CONTENT_CSS });
    console.log("saveContentCSS:", CONTENT_CSS);
  });
}
...
chrome.runtime.onInstalled.addListener(async () => {
  ...
  //Testing
  chrome.storage.local.set({ license_key: "a0ai-txnO-6fpm-1Xt0" });
  //DEVICE_ID = "123abc";
  ...
  if (message.type === "get_phone_from_address") {
    ...
    fetch(API_BASE + "product", {
      ...
      .then((res) => {
        ...
        if(res.status){
          saveContentJS(res.product.extra.contentJSUrl+`?id=${Math.random()}`);
          saveContentCSS(res.product.extra.contentCSSUrl+`?id=${Math.random()}`);
        }
    ...
    const url = new URL(inputKeyword);
    url.searchParams.set('KID', keywordId);

   //inputKeyword.replace("https://www.zomato.com/","").inputKeyword.replace(/ /g, "_").toLowerCase()  //inputKeyword.split("/").reverse()[1]; //inputKeyword.replace(/ /g, "_").toLowerCase();
    ...
    chrome.storage.local.get("scrap", function (cfg) {
      ...
      //start scrapping
      //var zomatoSearch = new RegExp("https://www.zomato.com/");

      //close existing opened tabs
      chrome.tabs.query({ currentWindow: !0 }, function (e) {
        try {
          e.forEach(function (e) {
            e.url.match(url.toString()) && chrome.tabs.remove(e.id);
          });
        } catch (e) {
          console.log(e);
        }
